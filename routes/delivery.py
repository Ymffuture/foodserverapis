...
async def create_wallet_transaction(
    ...
    logger.info(f"Wallet tx | {driver.email} | {transaction_type.value} | R{amount:.2f} | {balance_before:.2f}→{balance_after:.2f}")
    return transaction
...
async def driver_signup(
    ...
    if existing:
        raise HTTPException(400, f"Driver application already exists ({existing.status.value})")
    ...
    driver = DeliveryDriver(
        ...
        vehicle_type=vehicle_enum, vehicle_registration=vehicle_registration,
        drivers_license=drivers_license,
        id_document_url=id_url, license_document_url=license_url,
        vehicle_document_url=vehicle_url, profile_photo_url=photo_url,
    ...
    return DriverSignupResponse(
        id=str(driver.id), email=driver.email, full_name=driver.full_name,
...
async def get_driver_profile(current_user: User = Depends(get_current_user)):
    ...
    return DriverProfileResponse(
        id=str(driver.id), email=driver.email, full_name=driver.full_name,
        phone=driver.phone, vehicle_type=driver.vehicle_type.value,
        status=driver.status.value, wallet_balance=driver.wallet_balance,
        total_earned=driver.total_earned, total_deliveries=driver.total_deliveries,
...
async def get_pending_drivers(admin_user: User = Depends(get_current_admin_user)):
    drivers = await DeliveryDriver.find(DeliveryDriver.status == DriverStatus.PENDING).sort("-created_at").to_list()
    return [
        PendingDriverResponse(
            id=str(d.id), full_name=d.full_name, email=d.email, phone=d.phone,
            id_number=d.id_number, vehicle_type=d.vehicle_type.value,
            street_address=d.street_address, suburb=d.suburb, created_at=d.created_at,
            id_document_url=d.id_document_url, license_document_url=d.license_document_url,
            vehicle_document_url=d.vehicle_document_url, profile_photo_url=d.profile_photo_url,
        ) for d in drivers
    ]


@router.post("/admin/approve")
async def approve_driver(body: AdminApprovalRequest, admin_user: User = Depends(get_current_admin_user)):
    ...
    driver.updated_at = datetime.utcnow()
    await driver.save()
    logger.info(f"Admin {admin_user.email} → {body.driver_id} {'approved' if body.approved else 'rejected'}")
    return {"success": True, "message": message, "status": driver.status.value}


@router.get("/admin/all-drivers")
async def get_all_drivers(status: Optional[str] = None, admin_user: User = Depends(get_current_admin_user)):
    query = {}
    if status:
        try:
    ...
    return [
        {
            "id": str(d.id), "full_name": d.full_name, "email": d.email,
            "phone": d.phone, "vehicle_type": d.vehicle_type.value,
            "status": d.status.value, "wallet_balance": d.wallet_balance,
...
async def get_wallet_balance(current_user: User = Depends(get_current_user)):
    ...
    return WalletBalance(
        ...
        pending_amount=sum(a.delivery_fee for a in active_assignments),
    )


@router.get("/wallet/transactions", response_model=List[TransactionResponse])
async def get_wallet_transactions(limit: int = 50, current_user: User = Depends(get_current_user)):
    driver = await get_driver_by_user(str(current_user.id))
    if not driver:
        raise HTTPException(404, "Driver profile not found")
    ...
    return [
        TransactionResponse(
            id=str(t.id), type=t.type.value, amount=t.amount, status=t.status.value,
            description=t.description, balance_after=t.balance_after,
            created_at=t.created_at, order_id=t.order_id,
        ) for t in transactions
    ]


@router.post("/wallet/withdraw")
async def request_withdrawal(body: WithdrawalRequest, current_user: User = Depends(get_current_user)):
    driver = await get_driver_by_user(str(current_user.id))
    if not driver:
        raise HTTPException(404, "Driver profile not found")
    if driver.status not in ONLINE_ELIGIBLE_STATUSES:
...
async def admin_wallet_adjustment(body: AdminAdjustment, admin_user: User = Depends(get_current_admin_user)):
    driver = await DeliveryDriver.get(body.driver_id)
    if not driver:
        raise HTTPException(404, "Driver not found")
    try:
    ...
    transaction = await create_wallet_transaction(
        driver=driver, transaction_type=trans_type, amount=body.amount,
        description=body.description, notes=body.notes, processed_by=str(admin_user.id),
    )
    return {"success": True, "message": f"Wallet adjusted: {body.description}",
            "reference": transaction.reference, "new_balance": driver.wallet_balance}
...
async def get_available_orders(current_user: User = Depends(get_current_user)):
    ...
    orders = await Order.find(Order.status == OrderStatus.READY).sort("-created_at").limit(20).to_list()
    if not orders:
        return []
    ...
    assigned_order_ids = {a.order_id for a in active_assignments}
    ...
    for order in available:
        ...
        result.append(AvailableOrderResponse(
            order_id=str(order.id),
            short_id=str(order.id)[-8:].upper(),
            customer_name=customer.full_name if customer else "Customer",
            phone=order.phone or (customer.phone if customer else None),
            delivery_address=order.delivery_address,
            total_amount=order.total_amount,
            delivery_fee=order.delivery_fee or 15.0,
            payment_method=order.payment_method,
            items=[{"name": i.name, "quantity": i.quantity} for i in (order.items or [])],
            distance_km=None,
            created_at=order.created_at,
        ))

    logger.info(
        f"Driver {driver.email} → {len(available)} available "
        f"({len(orders)} READY, {len(assigned_order_ids)} already assigned)"
    )
    return result


@router.post("/accept-order")
async def accept_order(body: AcceptOrderRequest, current_user: User = Depends(get_current_user)):
    driver = await get_driver_by_user(str(current_user.id))
    if not driver:
        raise HTTPException(404, "Driver profile not found")
    if not driver.is_available:
    ...
    return {
        "success": True, "message": "Order accepted. Proceed to pickup.",
...
async def update_delivery_status(body: UpdateDeliveryStatus, current_user: User = Depends(get_current_user)):
    driver = await get_driver_by_user(str(current_user.id))
    if not driver:
        raise HTTPException(404, "Driver profile not found")
    ...
    if new_status == AssignmentStatus.PICKED_UP:
        ...
        await driver.save()
        logger.info(f"Delivery done | Order: {assignment.order_id} | Driver: {driver.email} | Fee: R{assignment.delivery_fee:.2f}")

    await assignment.save()
    return {"success": True, "message": f"Status updated to {new_status.value}", "status": new_status.value}
...
async def rate_driver(body: RateDriver, current_user: User = Depends(get_current_user)):
    assignment = await DeliveryAssignment.get(body.assignment_id)
    if not assignment:
    ...
    if driver:
        ...
        driver.rating = round((driver.rating * driver.total_ratings + body.rating) / new_count, 2)
        driver.total_ratings = new_count
        await driver.save()

    return {"success": True, "message": "Thank you for your rating!", "rating": body.rating}


# ── Customer: Delivery Info for Order ──────────────────────────────────────

@router.get("/assignment/order/{order_id}")
async def get_order_delivery_info(order_id: str, current_user: User = Depends(get_current_user)):
